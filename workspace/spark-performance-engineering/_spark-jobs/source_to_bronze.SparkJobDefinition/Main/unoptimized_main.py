"""
Spark Job Definition: Seed Delta tables for the performance workshop.
...
def main(argv):
    ...
    spark = create_spark("seedPerfWorkshopApp", args.debug)
    ...
    job_lock_uri = f"{lakehouse_root_uri}/Files/job_locks/spark_performance_engineering.lock"
    ...
    with job_lock(job_lock_uri):
        ...
        config = {
            ...
            "job_lock_enabled": False,  # Job lock is handled by a context manager that also wraps the data gen
            "job_id": "seed_lego_delta_tables",
            "job_lock_timeout_seconds": 60,
            "job_lock_path": f"{lakehouse_root_uri}/Files/job_locks",
            # Intentionally suboptimal — disable all write-path optimizations
            "spark_config_overrides": {
                "spark.databricks.delta.autoCompact.enabled": "false",
                "spark.microsoft.delta.autoCompact.onCheckpointOnly.enabled": "false",
                "spark.databricks.delta.optimizeWrite.enabled": "false",
                "spark.microsoft.delta.optimize.fast.enabled": "false",
                "spark.microsoft.delta.optimize.fileLevelTarget.enabled": "false",
                "spark.microsoft.delta.targetFileSize.adaptive.enabled": "false",
                "spark.databricks.delta.properties.defaults.enableDeletionVectors": "false",
                "spark.sql.parquet.vorder.enabled": "false",
                "spark.sql.parquet.native.writer.directWriteEnabled": "false",
