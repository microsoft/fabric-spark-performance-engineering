...
# # **Module 2 — Optimizing Tables (Spark SQL track)**
# 
...
# 
# > 🔀 **This is the Spark SQL track.** Every operation is expressed with `spark.sql("...")` — `OPTIMIZE`, `ALTER TABLE`, `INSERT INTO`, `DELETE`, `DESCRIBE HISTORY`, and window analytics — instead of the DataFrame API. If you prefer the fluent DataFrame API, use `dataframe-notebooks/02_optimizing-tables` instead. The concepts, tables, and benchmarks are identical.
# 
# 
# ### **Scenario**
# 
...
FAST_SCHEMA = "bronze_fast_sql"   # shallow clones we'll fix during the lab
...
# ============================================================
# Pure-SQL analysis: register DESCRIBE HISTORY as a temp view, then use SQL
# window functions to accumulate the file counts across commits.
print("🔍 Analyzing Delta transaction log for inventory_transaction...\n")

spark.sql(f"DESCRIBE HISTORY {ORIG_SCHEMA}.inventory_transaction").createOrReplaceTempView("inv_txn_history")

commit_stats_agg = spark.sql("""
    WITH base AS (
        SELECT
            version,
            timestamp,
            operation,
            CAST(operationMetrics['numOutputRows'] AS BIGINT) AS rows_written,
            COALESCE(
                CAST(operationMetrics['numFiles'] AS INT),
                CAST(operationMetrics['numAddedFiles'] AS INT)
            ) AS files_added
        FROM inv_txn_history
    ),
    filtered AS (
        SELECT *, ROW_NUMBER() OVER (ORDER BY version) AS iteration
        FROM base
        WHERE files_added IS NOT NULL AND files_added > 0
    ),
    grouped AS (
        SELECT *, FLOOR((iteration - 1) / 50) AS compaction_group
        FROM filtered
    )
    SELECT
        version,
        ...
        iteration,
        compaction_group,
        SUM(files_added) OVER (ORDER BY version ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)
            AS cumulative_files,
        -- Optimize Write alone would bin-pack each commit into a single file:
        SUM(1) OVER (ORDER BY version ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)
            AS simulated_files_if_ow_enabled,
        -- Optimize Write + Auto Compact would additionally coalesce every 50 commits:
        SUM(1) OVER (PARTITION BY compaction_group ORDER BY version ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)
            AS simulated_files_if_ow_and_ac_enabled,
        SUM(COALESCE(rows_written, 0)) OVER (ORDER BY version ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)
            AS cuml_rows_written
    FROM grouped
    ORDER BY version
""")
...
# The /*+ REPARTITION(8) */ hint mimics how streaming spreads a batch across
# multiple output partitions — one file per partition when optimize write is off.
BATCH_ROWS = 5000
...
optimize_metrics_df.createOrReplaceTempView("optimize_metrics_view")
print(f"   Completed in {round(time.time() - start, 1)}s")
...
# **Your task:**
# 
# 1. Drill into the `optimize_metrics_view` temp view to inspect the `skippingEffectiveness` of the clustering column(s).
# 
# > 💡 Hint: Use `EXPLODE` and `*` expand

# CELL ********************

# Extract clustering quality metrics from the optimize_metrics_view temp view
...
#     FROM (
#         SELECT EXPLODE(metrics.clusteringQuality) AS clustering_quality
#         FROM optimize_metrics_view
#     )
# """))
# ```
# 
...
total_count = spark.sql(f"SELECT COUNT(*) AS c FROM {FAST_SCHEMA}.web_order_line").collect()[0]["c"]
...
#
# CREATE TABLE ... AS SELECT with a /*+ REPARTITION(100) */ hint forces many small
# files (don't do this in production!) and TBLPROPERTIES sets the small target file size.
...
spark.sql(f"""
    CREATE TABLE {FAST_SCHEMA}.{WIDE_TABLE}
    USING DELTA
    TBLPROPERTIES ('delta.targetFileSize' = '1m')
    AS
...
wide_cols = spark.table(f"{FAST_SCHEMA}.{WIDE_TABLE}").columns
num_cols = len(wide_cols)
...
for i, col in enumerate(wide_cols, 1):
...
print(f"   part_num is at column position {wide_cols.index('part_num') + 1} (past the 32-col stats window)\n")

# How many files does the query scan?
total_files = len(spark.sql(f"SELECT * FROM {FAST_SCHEMA}.{WIDE_TABLE}").inputFiles())
...
try:
    ...
    if "DELTA_CLUSTERING_COLUMN_MISSING_STATS" in error_msg:
        ...
        print(f"   part_num is at position {wide_cols.index('part_num') + 1} \u2014 outside the stats window.")
...
print(f"\n\U0001f4cb Table has {len(wide_cols)} columns")
print(f"   Stats collected for:  columns 1-32")
print(f"   Stats MISSING for:   columns 33-{len(wide_cols)}")
print(f"   part_num position: {wide_cols.index('part_num') + 1} \u2190 not enabled for stats\u2192 clustering blocked")
