...
mfg = spark.sql("SELECT * FROM bronze.manufacturing_event")
mfg.createOrReplaceGlobalTempView("mfg")
...
# ============================================================
# 1️⃣ BENCHMARK — Capture baseline query time (SQL version)
# ============================================================

print("🐌 Running baseline query on manufacturing_event (SQL)...\n")

latest_day_str = str(latest_day)

with benchmark_op("Predicate Pushdown", "before", spark):
    result_q1 = spark.sql(f"""
        WITH mfg AS (
            SELECT manufacturing_event.*
            FROM bronze.manufacturing_event
        )
        SELECT
            SUBSTRING(timestamp, 1, 10) AS event_day,
            machine_id,
            COUNT(*) AS events,
            SUM(CAST(defect_detected AS INT)) AS defects,
            SUM(CAST(defect_detected AS INT)) / COUNT(*) AS defect_rate
        FROM mfg
        WHERE SUBSTRING(timestamp, 1, 10) = '{latest_day_str}'
        GROUP BY SUBSTRING(timestamp, 1, 10), machine_id
...
# ==================================================================================================
# 1️⃣ FIX — Use column functions (SQL version) to enable predicate pushdown
# ==================================================================================================
...
with benchmark_op("Predicate Pushdown", "after", spark):
    result_q1 = spark.sql(f"""
        ...
        SELECT
            ...
            SUM(CAST(defect_detected AS INT)) / COUNT(*) AS defect_rate
        FROM mfg
        WHERE timestamp LIKE '{latest_day_str}%'
        GROUP BY SUBSTRING(timestamp, 1, 10), machine_id
        ORDER BY defect_rate DESC
    """)
    data = result_q1.toPandas()
...
# ============================================================
# 2️⃣ BENCHMARK — Capture baseline query time with Python UDFs (SQL version)
# ============================================================

print("🐌 Running baseline query with Python UDFs (via SQL)...\n")
...
with benchmark_op("Python UDF Overhead", "Python UDFs", spark):
    # Create a temporary view projecting nested columns so we can call Python UDFs from SQL
    orders_df = spark.table("bronze.web_order").selectExpr("web_order.*")
    exploded_df = orders_df.select(
        F.col("customer_id"),
        F.col("order_date"),
        F.explode("order_lines").alias("line")
    ).withColumn(
        "line_total", python_line_total("line.quantity", "line.unit_price", "line.extended_price")
    ).withColumn(
        "order_day", python_extract_day("order_date")
    )

    exploded_df.createOrReplaceTempView("web_order_exploded_python_udf")
    ...
    result_q2 = spark.sql("""
        ...
        FROM (
            SELECT
                ...
                line_total,
                COUNT(*) AS line_count
            FROM web_order_exploded_python_udf
            GROUP BY customer_id, order_day, line_total
        ) t
...
# ==================================================================================================
# 2️⃣ FIX — Replace Python UDFs with native Spark SQL expressions (SQL version)
# ==================================================================================================

with benchmark_op("Python UDF Overhead", "Built-in Functions", spark):
    # Prepare flattened view using built-in expressions only
    orders_df = spark.table("bronze.web_order").selectExpr("web_order.*")
    exploded_df = orders_df.select(
        F.col("customer_id"),
        F.col("order_date"),
        ...
        "order_day", F.regexp_extract("order_date", r"(\d{4}-\d{2}-\d{2})", 1)
    )

    exploded_df.createOrReplaceTempView("web_order_exploded_builtin")

    result_q2_fixed = spark.sql("""
        SELECT
            customer_id,
        ...
        FROM (
            SELECT
                ...
                SUM(line_total) AS total_spend,
                COUNT(*) AS line_count
            FROM web_order_exploded_builtin
...
# ============================================================
# 3️⃣ BENCHMARK — Capture baseline query time with driver collect (SQL source)
# ============================================================

print("🐌 Running baseline query with driver-side collect (SQL source)...\n")

with benchmark_op("Local Driver work", "before", spark):
    # Use SQL to read the source, then collect to the driver and aggregate in Python
    inv_df = spark.sql("""
        SELECT
            line_id,
            part_num,
    ...
    result_q3 = pd.DataFrame(
        [{"line_id": line_id, "net_quantity": qty} for line_id, qty in inventory_by_line.items()]
...
# ==================================================================================================
# 3️⃣ FIX — Use distributed Spark aggregation instead of driver-side collect (SQL version)
# ==================================================================================================

print("✅ Running fixed query with distributed Spark aggregation (SQL)...\n")

with benchmark_op("Local Driver work", "after", spark):
    result_q3_fixed = spark.sql("""
        WITH inv AS (
            SELECT
                line_id,
                part_num,
...
# ============================================================
# 4️⃣ BENCHMARK — Capture baseline with repeated scans (no caching, SQL version)
# ============================================================

print("🐌 Running baseline with repeated scans (no caching, SQL)...\n")

with benchmark_op("Repeated Scans", "before", spark):
    # Base aggregation in SQL; we will requery it three times without caching
    base_sql = """
        SELECT
            p.name AS part_category,
            me.defect_detected,
            me.defect_type,
            COUNT(*) AS cnt
        FROM bronze.manufacturing_event AS me
        JOIN bronze.colors AS c
            ON me.color_id = c.id
        JOIN bronze.parts AS pa
            ON me.part_num = pa.part_num
        JOIN bronze.part_categories AS p
            ON pa.part_cat_id = p.id
        GROUP BY p.name, me.defect_detected, me.defect_type
    """

    # First action - full scan
    no_defect_df = spark.sql(f"""
        WITH mfg AS ({base_sql})
        SELECT part_category, SUM(cnt) AS total_count
        FROM mfg
        WHERE defect_detected = false
        GROUP BY part_category
    """)
    no_defects = no_defect_df.collect()

    # Second action - another full scan
    defect_df = spark.sql(f"""
        WITH mfg AS ({base_sql})
        SELECT part_category, SUM(cnt) AS total_count
        FROM mfg
        WHERE defect_type IN ('color_streak', 'warp', 'sink_mark', 'short_shot')
        GROUP BY part_category
    """)
    defects = defect_df.collect()

    # Third action - yet another full scan
    quarantine_df = spark.sql(f"""
        WITH mfg AS ({base_sql})
        SELECT part_category, SUM(cnt) AS total_count
        FROM mfg
        WHERE defect_type NOT IN ('color_streak', 'warp', 'sink_mark', 'short_shot')
        GROUP BY part_category
    """)
...
# ==================================================================================================
# 4️⃣ FIX — Cache expensive intermediate result before multiple actions (SQL version)
# ==================================================================================================

print("✅ Running fixed version with caching (SQL)...\n")

with benchmark_op("Repeated Scans", "after", spark):
    # 1) Materialize the expensive aggregation once using a GLOBAL TEMP VIEW.
    #    In Fabric, using a global temp view avoids catalog issues seen when caching
    #    regular TEMP VIEWs that reference lakehouse tables (e.g. bronze.*).
    mfg = spark.sql("""
        SELECT
            p.name AS part_category,
            me.defect_detected,
    ...
    no_defects_fixed = no_defect_df.groupBy("part_category").agg(F.sum("cnt").alias("total_count")).collect()

    # Second action - reads from cache (no re-scan)
    defect_df = mfg.filter(defect_type_col.isin(known_defects))
    defects_fixed = defect_df.groupBy("part_category").agg(F.sum("cnt").alias("total_count")).collect()

    # Third action - reads from cache (no re-scan)
    quarantine_df = mfg.filter(~defect_type_col.isin(known_defects))
    quarantine_rows_fixed = quarantine_df.groupBy("part_category").agg(F.sum("cnt").alias("total_count")).collect()

    mfg.unpersist()

    # 5) Capture physical plans for comparison with the non-cached version.
...
# ============================================================
# 5️⃣ BENCHMARK — Capture baseline with Cartesian join (SQL version)
# ============================================================

print("🐌 Running baseline with Cartesian join (no join predicate, SQL)...\n")
...
# ==================================================================================================
# 5️⃣ FIX — Add proper join predicate to create equi-join (SQL version)
# ==================================================================================================

print("✅ Running fixed query with proper join predicate (SQL)...\n")

with benchmark_op("Cartesian Join", "after", spark):
    result_q5_fixed = spark.sql("""
        WITH qi AS (
            SELECT
                quality_inspection.production_order_id AS qi_production_order_id,
                quality_inspection.result AS inspection_result,
        ...
        FROM qi
        JOIN po
            ON qi.qi_production_order_id = po.po_production_order_id
        GROUP BY po.machine_id
        ORDER BY joined_rows DESC
    """)
...
# ============================================================
# 6️⃣ BENCHMARK — Capture baseline with unnecessary cache (SQL source)
# ============================================================

print("🐌 Running baseline with unnecessary cache (SQL source)...\n")

with benchmark_op("Unnecessary Cache", "before", spark):
    # Use SQL to read the table, then cache via a temporary view-based DataFrame
    mfg_df = spark.sql("""
        SELECT *
        FROM bronze.manufacturing_event
    """)

    transformed = mfg_df.withColumn(
        "event_day",
        F.substring(F.col("manufacturing_event.timestamp"), 1, 10),
    )
    transformed.cache()  # Unnecessary cache - data is only used once

    result_q6 = (
        transformed.groupBy("event_day")
...
# ==================================================================================================
# 6️⃣ FIX — Remove unnecessary cache for single-use transformation (SQL source)
# ==================================================================================================

print("✅ Running fixed query without unnecessary cache (SQL source)...\n")
